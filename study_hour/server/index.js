...
app.get('/play-chess', (req, res) => {
    ...
    try {
        PythonShell.run('react/get_move.py', options, function (err, results) {
...
app.get('/validate_move', (req, res) => {
    ...
    try {
        PythonShell.run('engine/react/validate_move.py', options, function(err, results) {
            if (err) throw err;
        ...
        console.log("Error in Validate Move: " + err)
